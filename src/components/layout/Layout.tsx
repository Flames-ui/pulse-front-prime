    ...
    <header className={`fixed top-0 left-0 right-0 z-50 bg-[#0A0F1E] text-white border-b border-white/5 transition-transform duration-500 ease-in-out ${isHidden ? '-translate-y-full' : 'translate-y-0'}`}>
      <nav className="container mx-auto px-4 h-20 md:h-24 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-12">
          {links.map((link) => (
            <Link key={link.path} to={link.path} className={`text-[10px] font-black tracking-[0.2em] uppercase transition-all hover:text-[#0066FF] relative group ${location.pathname === link.path ? 'text-[#0066FF]' : 'text-gray-400'}`}>
              {link.name}
              <span className={`absolute -bottom-2 left-0 h-0.5 bg-[#FF6B00] transition-all duration-300 ${location.pathname === link.path ? 'w-full' : 'w-0 group-hover:w-full'}`} />
            </Link>
          ))}
          <div className="h-8 w-px bg-white/10 mx-2" />
          <Link to="/search" className="text-gray-400 hover:text-[#0066FF] transition-colors"><Search className="w-5 h-5" /></Link>
          ...
          {isOpen && (
            <motion.div initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: '100%' }} className="lg:hidden fixed inset-0 z-50 bg-[#0A0F1E] flex flex-col p-8 pt-24">
              ...
              <div className="flex flex-col gap-8 mb-12">
                {links.map((link) => (
                  <Link key={link.path} to={link.path} onClick={() => setIsOpen(false)} className="text-4xl font-black uppercase italic tracking-tighter hover:text-[#0066FF]">{link.name}</Link>
                ))}
              </div>
